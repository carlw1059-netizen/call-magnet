...
Deno.serve(async (req) => {
  ...
  if (userErr || !userData?.user) {
    return json(401, { ok: false, error: 'invalid_token', detail: userErr?.message ?? 'token did not resolve to a user' });
  }
  ...
  if (message.length > 160) {
    return json(400, { ok: false, error: 'message must be 160 characters or less', length: message.length });
  }
  if (message.toLowerCase().includes('callmagnet.com.au')) {
    return json(400, { ok: false, error: 'message must not contain callmagnet.com.au' });
  }

  // Normalise AU mobile: 04xx → +614xx
  const toNormalised = to.replace(/\s+/g, '').replace(/^0/, '+61');
  if (!/^\+614\d{8}$/.test(toNormalised)) {
    return json(400, { ok: false, error: 'to must be an Australian mobile in +614XXXXXXXX format', received: toNormalised });
  }

  // ── 4. Resolve from number if not supplied ───────────────────────────────
  if (!from) {
