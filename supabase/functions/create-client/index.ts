  ...
  try {
    ...
    if (pricing_package !== 'free_trial') {
      try {
        ...
        // ── Payment-mode checkout session for setup fee (card saved for future subscription) ──
        // Restaurant: $499 setup fee. Hairdresser: $249 setup fee.
        // Subscription is created later by activate-client (called manually by Carl after setup).
        const setupPriceId = pricing_package === 'restaurant'
          ? 'price_1Ti51s3MTu8r2rLhmmtEk3Fb'
          : 'price_1TD0jm3MTu8r2rLhkXPpx0AH';
        ...
        const csParams = new URLSearchParams({
          ...
          'metadata[client_id]':                     client_id,
          'metadata[slug]':                          slug,
          'metadata[pricing_package]':               pricing_package,
        ...
        console.log(`create-client: ${pricing_package} checkout session created — ${checkout_session_id}`);
      } catch (e) {
